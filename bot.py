...
class ShadowClient(discord.Client):
	...
	async def quit_game(self,channel):
		...
		self.main_channel_name = None
		prelaunch.stop()
		self.game.clean()
		await channel.send('Arrêt de la partie.')

	async def launch_game(self,channel):
		prelaunch.launch()
		self.joining_message = None
		self.game = main.Game(prelaunch.players())
		await channel.send('**Le jeu commence maintenant !**')
		await channel.send(self.game.players_order_str())
		await channel.send(self.game.get_board())
	...
	async def on_message(self,message):
		...
		if message.content == '!board':
			...
			if (not prelaunch.launched()):
				...
				await message.channel.send(self.game.get_board())
		...
		if message.content == '!order':
			...
			if (not prelaunch.launched()):
				...
				await message.channel.send(self.game.players_order_str())
		...
		if message.content == '!players':
			...
			if (not prelaunch.launched()):
				...
				await message.channel.send(self.game.get_players_state())
		...
		if message.content == '!reveal':
			...
			if (not prelaunch.launched()):
				...
				try:
					await message.channel.send(self.game.reveal_player(message.author))
				except exceptions.PlayerNotFound:
		...
		if message.content == '!start':
			...
			elif str(message.channel) != self.main_channel_name:
			...
			# If not enough players
				...
				await self.launch_game(message.channel)
				turn_message = await message.channel.send('Tour de '+self.game.getMention(0))
